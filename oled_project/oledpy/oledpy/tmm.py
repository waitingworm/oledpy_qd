...
from numpy import cos, inf, zeros, array, exp, conj, nan, isnan, pi, sin, sqrt
...
def load_nk(layers,doping,wavelength_nm,df_nk):
    '''
    ...
    n_list = []
    assert isinstance(wavelength_nm, (int, float)), 'wavelength_nm should be int or float'
    assert len(layers)==len(doping), 'doping and layers need same length'
    lambda_nm = df_nk['Wavelength (nm)']
    for idx,layer in enumerate(layers):
        # If layer is doped, calculate weighted avg n and k
        if type(doping[idx]) is list:
            n = 0
            k = 0
            for idx2,doped_name in enumerate(layer.split('-')):
                n = n + np.interp(wavelength_nm,lambda_nm,
                                  df_nk[doped_name+'_n'])*doping[idx][idx2]
                k = k + np.interp(wavelength_nm,lambda_nm,
                                  df_nk[doped_name+'_k'])*doping[idx][idx2]
        else:
            n = np.interp(wavelength_nm,lambda_nm,df_nk[layer+'_n'])
            k = np.interp(wavelength_nm,lambda_nm,df_nk[layer+'_k'])
...
def coherent_tmm(pol, n_list, d_list, th_0, lam_vac):
    ...
    * pol, n_list, d_list, th_0, lam_vac--same as input
    ...
    return {'r': r, 't': t, 'R': R, 'T': T, 'power_entering': power_entering,
           ...
           'Mtilde':Mtilde, 'I_list':I_list, 'L_list':L_list}
...
def position_resolved(layer, distance, coh_tmm_data, powerDensity=1):
    """
    ...
    * Ex and Ey and Ez - the electric field amplitudes, where
    ...
    lam_vac = coh_tmm_data['lam_vac']
    assert lam_vac > 299, ('Check units on wavelength, should be in nm, '+
                         'Calculation of generation rate assumes this')
    ...
    # Electric field
    if pol == 's':
        Ex = np.zeros(distance.size)
        Ey = Ef + Eb
        Ez = np.zeros(distance.size)
    elif pol == 'p':
        Ex = (Ef - Eb) * cos(th)
        Ey = np.zeros(distance.size)
        Ez = (-Ef - Eb) * sin(th)
    # The norm squared of the electric field is the physical quantity we care about,
    # as absorption is proportional to this
    Esq = (Ex.real**2 + Ex.imag**2 + Ey.real**2 + Ey.imag**2 + 
                      Ez.real**2 + Ez.imag**2)
    # Calculate the generation rate # of excitons / cm^3 / s
    h = 6.626e-34 # J-s, planck constant
    c = 2.99e8 #m/s, speed of light
    eps0 = 8.854e-12 # F/m or C^2/(Nm^2), vacuum permittivity
    # Absorption coefficient, 1/cm, assumes lam_vac in nm
    alpha = 4*pi*n.imag/(lam_vac*1e-7) 
    # See eqn 22, 23 and 26 of Pettersson
    # Assumes quantum efficiency of exciton generation is 100%
    cm3 = 1e6 # cm^3/m^3
    # Q - Energy dissipation rate in mW/cm^3 or J/(cm^3-s)
    # powerDensity = 0.5 * C * eps0 * E_0^2
    # Note that Esq is already normalized to E_0 = 1
    Q = powerDensity * alpha * (n.real / n_0.real) * Esq
    # Generation rate in # of excitons / (cm^3-s)
    mW = 1e-3 # converts mW to W
    G = Q * mW / (h*c/(lam_vac*1e-9))
    return {'poyn': poyn, 'absor': absor, 'Ex': Ex, 'Ey': Ey, 'Ez': Ez, 'Esq':Esq, 'G':G}

def position_multilayer(layers,coh_tmm_data,d_step=1,powerDensity=1):
    # Get position dependences across all 'layers'
    d_list = coh_tmm_data['d_list']
    d_list[d_list == inf] = 0 # Get rid of infinite distances
    assert 0 not in layers, 'Cannot use semi-infinite layer'
    assert len(d_list)-1 not in layers, 'Cannot use semi-infinite layer'
    total_distance = np.sum(d_list[layers])
    distance_from_first_layer = np.cumsum(d_list)-np.sum(d_list[:layers[0]])
    # Add length of layerst to account for overlapping points
    array_size = int(np.floor(total_distance / d_step))+len(layers)
    Ex = np.empty(array_size,dtype=complex)
    Ey = np.empty(array_size,dtype=complex)
    Ez = np.empty(array_size,dtype=complex)
    Esq = np.zeros(array_size)
    G = np.zeros(array_size)
    z_total = np.zeros(array_size)
    f_idx = 0
    for layer in layers:
        position = np.arange(0,d_list[layer]+d_step,d_step)
        position_data = position_resolved(layer,position,coh_tmm_data,powerDensity)
        s_idx = f_idx
        f_idx = s_idx+position.size
        Ex[s_idx:f_idx]=position_data['Ex']
        Ey[s_idx:f_idx]=position_data['Ey']
        Ez[s_idx:f_idx]=position_data['Ez']
        Esq[s_idx:f_idx]=position_data['Esq']
        G[s_idx:f_idx]=position_data['G']
        z_total[s_idx:f_idx]=position+distance_from_first_layer[layer-1]
    return {'z_total':z_total, 'Ex':Ex,'Ey':Ey,'Ez':Ez,'Esq':Esq,'G':G}
    
def plot_Efield(layers,coh_tmm_data,d_step=1,legLabel='',color=None,fig=None,ax=None,layerNames=None):
    """
    Make a plot of E-field vs. position for a collection of layers
    """
    d_list = coh_tmm_data['d_list']
    distance_from_first_layer = np.cumsum(d_list)-np.sum(d_list[:layers[0]])
    position_data = position_multilayer(layers,coh_tmm_data,d_step=1)
    z = position_data['z_total']
    E = position_data['Esq']
    if not (fig and ax):
        fig, ax = plt.subplots(facecolor="white")
    if not color:
        color = 'r'
    ax.plot(z,E,label=legLabel,linewidth=2,color=color)
    if len(layers)>1:
        for layer in layers:
            ax.axvline(distance_from_first_layer[layer-1],
                       color='k',linestyle='--',alpha=0.5)
    if legLabel:
        plt.legend(loc=0)
    if layerNames:
        for idx,name in enumerate(layerNames):
            if d_list[layers[idx]]>25:
                x = (distance_from_first_layer[layers[idx]-1] + 
                 distance_from_first_layer[layers[idx]])/2
                rot=0
            else:
                x = distance_from_first_layer[layers[idx]-1]
                rot=90
            plt.text(x,np.amax(E)+0.3,name,rotation=rot)
