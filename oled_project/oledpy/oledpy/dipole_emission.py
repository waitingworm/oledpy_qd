...
class ThinFilmArchitecture(object):
    ...
    def load_qled_materials(self, material_db):
        self.qd_n = np.interp(self.wavelengths, 
                            material_db['Wavelength'], 
                            material_db['CdSe/ZnS_QD_n'])
        self.qd_k = np.interp(self.wavelengths,
                            material_db['Wavelength'],
                            material_db['CdSe/ZnS_QD_k'])
        self.layer_stack[3]['n'] = self.qd_n + 1j*self.qd_k  # QD 층 업데이트